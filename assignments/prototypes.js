...
Villain.prototype.attack = function (opponent, damage) {
    ...
    if (opponent.healthPoints <= 0) {
        opponent.destroy();
        return `${this.name} slashed ${opponent.name} with a ${this.weapons[0]} for ${damage} health points and defeated them.`;
    } else {
        return `${this.name} slashed ${opponent.name} with a ${this.weapons[0]} for ${damage} health points.`;
    }
}
